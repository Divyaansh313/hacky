...
if not os.path.isfile("con.py"):
	sys.exit("'con.py' not found! Please add it and try again.")
else:
	import con
...
db_file = f"{directory}/files/hacky.db"
db = database.Database(db_file)

if __name__ == "__main__":
	for extension in con.STARTUP_COGS:
		try:
...
async def updates():
    ...
    if new_version:
        ...
        em = discord.Embed(
            title=f"Hacky v{new_version} - Changes",
        ...
        await system_notification(
            None,
            f"An update is available. Download Hacky **v{new_version}** at"
            f" <https://github.com/kcoder63/hacky> or simply use `{prefix}update`"
...
async def new(ctx):
    if isadmin(ctx.message.author, ctx.guild.id):
        sent_initial_message = await ctx.send(
            "Welcome to the Hacky's role creation. Please provide the required information once requested. If you would like to abort the creation, do not respond and the program will time out."
        )
        rl_object = {}
        cancelled = False
        ...
        if not cancelled:
            ...
            try:
                ...
                await ctx.author.send(
                    "Hacky's role creation failed, you took too long to provide the requested information."
                )
                cancelled = True
            finally:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            error_messages.append(
                                (
                                    await ctx.send(
                                        f"This message already got an instance attached to it, consider running `{prefix}edit` instead."
                                    )
                                )
                            )
                except asyncio.TimeoutError:
                    await ctx.author.send(
                        "hacky failed, you took too long to provide the requested information."
                    )
                    cancelled = True
                finally:
        ...
        if not cancelled:
            ...
            try:
                ...
                await ctx.send(
                    f"The requested message already got an instance attached to it, consider running `{prefix}edit` instead."
                )
                return
...
async def hlp(ctx):
    if isadmin(ctx.message.author, ctx.guild.id):
        ...
        await ctx.send(
            ...
            f"{botname} is running version {__version__} of Hacky. You can"
            " find more resources, submit feedback, and report bugs at: "
            "<https://github.com/kcoder63/hacky>"
        )
